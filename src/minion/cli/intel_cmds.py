...
def register_commands(cli: click.Group) -> None:
    ...
    @click.option("--slug", "-s", required=True, help="Human-readable key for this doc")
    @click.option("--path", "-p", "doc_path", required=True, help="Absolute path to the .md file")
    @click.option("--tags", "-t", default="", help="Comma-separated list of tags")
    @click.option("--description", "-d", default="", help="Short description")
    @click.option("--created-by", "-c", default="", help="Agent or user who created this doc")
    @click.option("--scaffold", "-S", is_flag=True, default=False, help="Create file with frontmatter stub if missing")
    @click.pass_context
    ...
    @click.option("--slug", "-s", required=True, help="Intel doc slug")
    @click.option("--task", "-t", "task_id", type=int, default=None, help="Task ID to link")
    @click.option("--req", "-r", "req_id", type=int, default=None, help="Requirement ID to link")
    @click.pass_context
    ...
    @click.option("--tag", "-t", default="", help="Filter by tag")
    @click.option("--path", "-p", "path_fragment", default="", help="Filter by path fragment")
    @click.pass_context
    ...
    @click.option("--slug", "-s", required=True, help="Intel doc slug")
    @click.option("--summary", "-S", is_flag=True, default=False, help="Return first 10 lines only")
    @click.pass_context
    ...
    @click.option("--task-id", "-t", required=True, type=int, help="Task ID to look up intel for")
    @click.pass_context
    ...
    @click.option("--topic", "-T", default="", help="Topic keywords to search for")
    @click.option("--task-id", "-t", default=None, type=int, help="Infer keywords from task title")
    ...
    @click.option("--scan-dir", "-d", default="docs", help="Directory to scan (default: docs/)")
    @click.option("--created-by", "-c", default="register-docs", help="Agent or user registering")
    @click.pass_context
