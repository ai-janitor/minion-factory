...
def _lineage_to_dict(lineage: dict) -> dict:
    ...
    for req_entry in lineage.get("requirements", []):
        ...
        for task_entry in req_entry.get("tasks", []):
            ...
            tasks.append({"task": task_dict, "transitions": transitions})
        result["requirements"].append({"req": req_dict, "tasks": tasks})
...
def _build_lineage(db_path: str, backlog_id: int) -> dict[str, Any]:
    ...
    """
    from minion.db.connection import connect
    from minion.dashboard.queries import fetch_lineage
    ...
    return {"type": "lineage", "data": _lineage_to_dict(lineage)}
...
def _http_handler(connection: Any, request: Any) -> Any:
    ...
    # Let WebSocket connections through (return None = proceed with WS upgrade)
    return None
